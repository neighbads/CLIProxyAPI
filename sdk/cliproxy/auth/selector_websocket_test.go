...
func TestSchedulerCodexWebsocketAffinityFallsBackToRoundRobin(t *testing.T) {
	headers := http.Header{}
	headers.Set("X-Claude-Code-Session-Id", "session-a")
	opts := cliproxyexecutor.Options{
		SourceFormat: sdktranslator.FromString("claude"),
	...
	scheduler := newSchedulerForTest(
		&RoundRobinSelector{},
		&Auth{ID: "http-a", Provider: "codex"},
		&Auth{ID: "http-b", Provider: "codex"},
		&Auth{ID: "websocket", Provider: "codex", Attributes: map[string]string{"websockets": "true"}},
	)
	tried := map[string]struct{}{"websocket": {}}

	first, errPick := scheduler.pickSingle(context.Background(), "codex", "", opts, tried)
	if errPick != nil {
		t.Fatalf("first pickSingle() error = %v", errPick)
	}
	second, errPick := scheduler.pickSingle(context.Background(), "codex", "", opts, tried)
	if errPick != nil {
		t.Fatalf("second pickSingle() error = %v", errPick)
	}
	if first == nil || second == nil || first.ID == second.ID {
		t.Fatalf("HTTP fallback picks = %#v, %#v; want round-robin", first, second)
	}
}
